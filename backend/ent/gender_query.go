...
func (gq *GenderQuery) Clone() *GenderQuery {
	...
	return &GenderQuery{
		...
		withPatients: gq.withPatients.Clone(),
...
func (gq *GenderQuery) sqlAll(ctx context.Context) ([]*Gender, error) {
	...
	if query := gq.withPatients; query != nil {
		...
		for _, n := range neighbors {
			fk := n.gender_patients
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "gender_patients" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "gender_patients" returned %v for node %v`, *fk, n.ID)
			}
