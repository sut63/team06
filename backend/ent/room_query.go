...
type RoomQuery struct {
	...
	// eager-loading edges.
	withRoomToAppointmentResults *AppointmentResultsQuery
...
// QueryRoomToAppointmentResults chains the current query on the "RoomToAppointmentResults" edge.
func (rq *RoomQuery) QueryRoomToAppointmentResults() *AppointmentResultsQuery {
	query := &AppointmentResultsQuery{config: rq.config}
...
func (rq *RoomQuery) Clone() *RoomQuery {
	...
	return &RoomQuery{
		...
		withRoomToAppointmentResults: rq.withRoomToAppointmentResults.Clone(),
...
// WithRoomToAppointmentResults tells the query-builder to eager-load the nodes that are connected to
// the "RoomToAppointmentResults" edge. The optional arguments are used to configure the query builder of the edge.
func (rq *RoomQuery) WithRoomToAppointmentResults(opts ...func(*AppointmentResultsQuery)) *RoomQuery {
	query := &AppointmentResultsQuery{config: rq.config}
	for _, opt := range opts {
		opt(query)
	}
	rq.withRoomToAppointmentResults = query
	return rq
}
...
func (rq *RoomQuery) sqlAll(ctx context.Context) ([]*Room, error) {
	var (
		...
		_spec       = rq.querySpec()
		loadedTypes = [1]bool{
			rq.withRoomToAppointmentResults != nil,
		}
	)
	...
	if query := rq.withRoomToAppointmentResults; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Room)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.RoomToAppointmentResults = []*AppointmentResults{}
		}
		query.withFKs = true
		query.Where(predicate.AppointmentResults(func(s *sql.Selector) {
			s.Where(sql.InValues(room.RoomToAppointmentResultsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.room_room_to_appointment_results
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "room_room_to_appointment_results" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "room_room_to_appointment_results" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.RoomToAppointmentResults = append(node.Edges.RoomToAppointmentResults, n)
		}
	}
