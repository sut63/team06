...
func (tx *Tx) init() {
	tx.AppointmentResults = NewAppointmentResultsClient(tx.config)
	...
	tx.MedicalProcedure = NewMedicalProcedureClient(tx.config)
	...
	tx.RightToTreatmentType = NewRightToTreatmentTypeClient(tx.config)
	tx.Room = NewRoomClient(tx.config)
	tx.TreatmentType = NewTreatmentTypeClient(tx.config)
	tx.TriageResult = NewTriageResultClient(tx.config)
	tx.UrgencyLevel = NewUrgencyLevelClient(tx.config)
}
...
// applies a query, for example: AppointmentResults.QueryXXX(), the query will be executed
