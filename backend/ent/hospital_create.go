...
func (hc *HospitalCreate) AddHospitalIDs(ids ...int) *HospitalCreate {
	hc.mutation.AddHospitalIDs(ids...)
	return hc
}

// AddHospital adds the "hospital" edges to the RightToTreatment entity.
func (hc *HospitalCreate) AddHospital(r ...*RightToTreatment) *HospitalCreate {
	ids := make([]int, len(r))
	for i := range r {
		ids[i] = r[i].ID
	}
	return hc.AddHospitalIDs(ids...)
}
...
func (hc *HospitalCreate) check() error {
	if _, ok := hc.mutation.HospitalName(); !ok {
		return &ValidationError{Name: "HospitalName", err: errors.New("ent: missing required field \"HospitalName\"")}
	}
...
func (hc *HospitalCreate) createSpec() (*Hospital, *sqlgraph.CreateSpec) {
	...
	if nodes := hc.mutation.HospitalIDs(); len(nodes) > 0 {
